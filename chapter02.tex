...
document-class为命令名，只能用英文字母(一些\TeX 原生的命令不遵守此规定)。这个规定涉及到\TeX 中的字符分类码(code
...
\LaTeX 命令由\texinline{\}开始，一般来讲命令只能由字母组成。因为\LaTeX 是由\TeX 的基础上发展而来，所以兼容\TeX
的所有命令，一些\TeX
原生的命令并不遵守这个规定，比如\texinline{\"}给字母上面加上两点。中括号内放置的是可选参数，因为是可选参数也就意味着命令内部有默认值。花括号内的是必选参数，就是说这对花括号必须要出现，但是内容可以为空。根据命令的不同，可选参数的个数会不同。
